...
from jssp import build_model, plot_gantt_schedule, plot_worker_utilization_interactive, solve_model, generate_output, plot_schedule, plot_worker_utilization
import io
import base64
...
def index():
    if request.method == 'POST':
        ...
        model = build_model(param1, param2, param3, param4, hard_deadline=hard_deadline)
        ...
        schedule_fig = plot_gantt_schedule(model, show=False, mode='workers')
        worker_util_fig = plot_worker_utilization_interactive(model, show=False)
        # plot_worker_utilization(model, show=False)

        return render_template('results.html', projects=output_data['projects'], workers=output_data['workers'], additional_info=output_data['additional_info'], schedule_fig=schedule_fig, worker_util_fig=worker_util_fig)
